...
impl FromStr for Elf {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let parsed_snacks =
            s.lines().map(|l| l.parse::<u32>()).collect::<Result<Vec<_>, _>>()?;
        Ok(Elf { snacks: parsed_snacks })
    }
}
...
fn parse_input(input : &str) -> Input {
    input.split("\n\n").map(|e| e.parse::<Elf>().unwrap()).collect()
}
...
fn top3_calories_carried(input: &Input) -> Option<u32> {
    Some(input.iter().map(Elf::count_calories).sorted_by_key(|c| Reverse(*c)).take(3).sum())
}
...
mod tests {
    ...
    fn test_most_calories_carried() {
        let input = vec![
            Elf { snacks: vec![1000, 2000, 3000] },
            Elf { snacks: vec![4000] },
            Elf { snacks: vec![5000, 6000] },
            Elf { snacks: vec![7000, 8000, 9000] },
            Elf { snacks: vec![10000] },
        ];
